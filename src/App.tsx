...
const formatMeso = (value: number) => `${currency.format(value)} 메소`;
const formatCombatPower = (value: number) =>
  value >= 100_000_000
    ? `${(value / 100_000_000).toFixed(1)}억`
    : value >= 10_000
      ? `${Math.round(value / 10_000)}만`
      : currency.format(value);
const formatRequiredCombatPower = (value: number | null) => (value === null ? "미정" : formatCombatPower(value));
const getDifficultyLabel = (difficulty: string) => ({ EASY: "이지", NORMAL: "노말", HARD: "하드", CHAOS: "카오스" }[difficulty] ?? difficulty);
...
function App() {
  ...
  return <div className="app-shell"><div className="aurora aurora-left" /><div className="aurora aurora-right" /><header className="hero"><div className="hero-copy"><p className="eyebrow">Maple Weekly Operations</p><h1>메이플 보스 대시보드</h1><div className="hero-meta"><span>{isCurrentWeekSelected ? "현재 주간" : "조회 주간"}</span><strong>{selectedWeekKey}</strong><em>{formatWeekRange(selectedWeekDate)}</em></div></div><div className="hero-panel"><label className="week-selector"><span>주간 선택</span><select value={selectedWeekKey} onChange={(event) => setSelectedWeekKey(event.target.value)}>{availableWeekKeys.map((weekKey) => <option key={weekKey} value={weekKey}>{weekKey === currentWeekKey ? `${weekKey} (현재)` : weekKey}</option>)}</select></label><div className="signal-card"><span>전체 주간 수익</span><strong>{formatMeso(dashboardStats.totalWeeklyIncome)}</strong></div><div className="signal-grid"><article><span>캐릭터 수</span><strong>{dashboardStats.totalCharacterCount}</strong></article><article><span>주간 클리어 수</span><strong>{dashboardStats.totalWeeklyClearCount}</strong></article></div></div></header><main className="dashboard-grid"><section className="panel roster-panel"><div className="panel-heading"><div><p className="eyebrow">Roster</p><h2>캐릭터 목록</h2></div><div className="panel-actions"><button className={`collapse-button ${isCharacterEditMode ? "is-active" : ""}`} type="button" onClick={() => setIsCharacterEditMode((current) => !current)}>{isCharacterEditMode ? "수정 모드 ON" : "수정 모드"}</button><button className="collapse-button add-button" type="button" onClick={openCharacterCreateModal}>추가</button></div></div>{characterSummaries.length === 0 ? <div className="empty-state"><strong>등록된 캐릭터가 없습니다.</strong><p>캐릭터를 추가하면 주간 보스 체크와 수익 집계를 바로 확인할 수 있습니다.</p></div> : <div className="roster-strip">{characterSummaries.map((summary) => { const isSelected = summary.character.characterId === selectedCharacterId; const isComplete = summary.clearCount === 12; return <div key={summary.character.characterId} className={`roster-card ${isSelected ? "selected" : ""}`}><button className="roster-main" onClick={() => setSelectedCharacterId(summary.character.characterId)} type="button"><div className="roster-title"><div><strong>{summary.character.characterName} / {summary.character.jobName}</strong></div></div><div className="roster-metrics-grid compact"><div className="metric-box"><span>전투력</span><strong>{formatCombatPower(summary.character.combatPower)}</strong></div><div className={`metric-box clear-state ${isComplete ? "is-complete" : "is-incomplete"}`}><span>클리어 현황</span><strong>{summary.clearCount} / 12</strong><em>{isComplete ? "완료" : "진행 중"}</em></div><div className="metric-box metric-box-wide"><span>수익</span><strong>{formatMeso(summary.weeklyIncome)}</strong></div></div></button>{isCharacterEditMode ? <div className="card-actions"><button className="secondary-button" type="button" onClick={() => startEditingCharacter(summary.character)}>수정</button><button className="danger-button" type="button" onClick={() => deleteCharacter(summary.character.characterId)}>삭제</button></div> : null}</div>; })}</div>}</section><section className="panel detail-panel"><div className="panel-heading"><div><p className="eyebrow">Character Detail</p><h2>{selectedCharacter?.characterName ?? "캐릭터를 선택해주세요"}</h2></div></div>{selectedSummary ? <><div className="detail-summary compact-detail"><article><span>직업</span><strong>{selectedSummary.character.jobName}</strong></article><article><span>전투력</span><strong>{formatCombatPower(selectedSummary.character.combatPower)}</strong></article><article><span>주간 수익</span><strong>{formatMeso(selectedSummary.weeklyIncome)}</strong></article><article><span>주간 클리어</span><strong>{selectedSummary.clearCount} / 12</strong></article></div><div className="boss-columns compact-columns"><div className="boss-column wide"><div className="column-title"><h3>클리어한 보스</h3><span>{selectedSummary.clearedBosses.length}회</span></div><div className="boss-chip-list">{selectedSummary.clearedBosses.length === 0 ? <span className="boss-chip">아직 클리어한 보스가 없습니다.</span> : selectedSummary.clearedBosses.map((boss) => <span key={boss.bossId} className="boss-chip cleared">{boss.bossName}</span>)}</div></div></div>{editingBossId ? <section className="boss-admin"><div className="panel-heading"><div><p className="eyebrow">Boss Admin</p><h2>보스 정보 수정</h2></div></div><form className="boss-form" onSubmit={handleBossSubmit}><div className="boss-form-grid"><label><span>보스</span><select value={bossForm.bossId} onChange={(event) => { const nextBoss = bosses.find((boss) => boss.bossId === event.target.value); setBossForm({ bossId: event.target.value, requiredCombatPower: nextBoss?.requiredCombatPower === null ? "" : String(nextBoss?.requiredCombatPower ?? ""), crystalPrice: nextBoss ? String(nextBoss.crystalPrice) : "" }); }}><option value="">보스를 선택하세요</option>{bossOptions.map((boss) => <option key={boss.bossId} value={boss.bossId}>{boss.bossName} / {getDifficultyLabel(boss.difficulty)}</option>)}</select></label><label><span>요구 전투력</span><input inputMode="numeric" value={bossForm.requiredCombatPower} onChange={(event) => setBossForm((current) => ({ ...current, requiredCombatPower: sanitizeNumericInput(event.target.value) }))} placeholder="예: 180000000" /></label><label><span>결정석 가격</span><input inputMode="numeric" value={bossForm.crystalPrice} onChange={(event) => setBossForm((current) => ({ ...current, crystalPrice: sanitizeNumericInput(event.target.value) }))} placeholder="예: 396000000" /></label></div><div className="form-actions"><button type="submit">저장</button><button className="secondary-button" type="button" onClick={resetBossForm}>취소</button></div></form></section> : null}{unknownBosses.length > 0 ? <div className="notice-box"><strong>요구 전투력 미정 보스 {unknownBosses.length}개</strong><p>요구 전투력이 없는 보스는 입장 가능 여부를 계산하지 않습니다.</p></div> : null}{!isCurrentWeekSelected ? <div className="notice-box"><strong>이전 주간 조회 중</strong><p>지난 주간 기록은 조회만 가능하며 현재 주간만 수정할 수 있습니다.</p></div> : null}<div className="boss-toolbar"><label className="boss-search"><span>보스 검색</span><input value={bossSearch} onChange={(event) => setBossSearch(event.target.value)} placeholder="보스 이름 또는 난이도로 검색" /></label><div className="boss-filter-group" role="group" aria-label="보스 난이도 필터">{[{ value: "ALL", label: "전체" }, { value: "EASY", label: "이지" }, { value: "NORMAL", label: "노말" }, { value: "HARD", label: "하드" }, { value: "CHAOS", label: "카오스" }].map((filter) => <button key={filter.value} className={`filter-chip ${bossDifficultyFilter === filter.value ? "is-active" : ""}`} type="button" onClick={() => setBossDifficultyFilter(filter.value as BossDifficultyFilter)}>{filter.label}</button>)}</div></div><div className="boss-table-wrap"><table className="boss-table"><thead><tr><th>체크</th><th>보스</th><th>난이도</th><th>요구 전투력</th><th>입장 가능</th><th>결정석 가격</th><th>수정</th></tr></thead><tbody>{selectedBossRows.length === 0 ? <tr><td colSpan={7} className="boss-table-empty">조건에 맞는 보스가 없습니다.</td></tr> : selectedBossRows.map(({ boss, isEligible, isCleared }) => <tr key={boss.bossId}><td><label className="checkbox-wrap"><input checked={isCleared} disabled={!isCurrentWeekSelected} onChange={() => toggleBossClear(boss.bossId)} type="checkbox" /><span>{isCleared ? "클리어" : "미클리어"}</span></label></td><td>{boss.bossName}</td><td><span className={`difficulty-badge ${getDifficultyClassName(boss.difficulty)}`}>{getDifficultyLabel(boss.difficulty)}</span></td><td>{formatRequiredCombatPower(boss.requiredCombatPower)}</td><td><span className={`eligibility ${isEligible === null ? "unknown" : isEligible ? "good" : "bad"}`}>{isEligible === null ? "미정" : isEligible ? "가능" : "부족"}</span></td><td>{formatMeso(boss.crystalPrice)}</td><td><button className="secondary-button" type="button" onClick={() => startEditingBoss(boss)}>수정</button></td></tr>)}</tbody></table></div></> : <div className="empty-state"><strong>선택된 캐릭터가 없습니다.</strong><p>캐릭터를 추가하거나 목록에서 선택해 주세요.</p></div>}</section></main>{isCharacterFormOpen ? <div className="modal-overlay" role="presentation" onClick={resetCharacterForm}><div className="modal-panel" role="dialog" aria-modal="true" aria-labelledby="character-modal-title" onClick={(event) => event.stopPropagation()}><div className="panel-heading"><div><p className="eyebrow">Character Setup</p><h2 id="character-modal-title">{editingCharacterId ? "캐릭터 수정" : "캐릭터 등록"}</h2></div><button className="collapse-button" type="button" onClick={resetCharacterForm}>닫기</button></div><form className="setup-form modal-form" onSubmit={handleCharacterSubmit}><div className="form-heading"><h3>{editingCharacterId ? "캐릭터 정보 수정" : "새 캐릭터 추가"}</h3><span>캐릭터 이름, 직업, 전투력을 입력하세요.</span></div><label><span>캐릭터 이름</span><input value={characterForm.characterName} onChange={(event) => setCharacterForm((current) => ({ ...current, characterName: event.target.value }))} placeholder="예: 메르세데스" /></label><label><span>직업</span><input value={characterForm.jobName} onChange={(event) => setCharacterForm((current) => ({ ...current, jobName: event.target.value }))} placeholder="예: 히어로" /></label><label><span>전투력</span><input inputMode="numeric" value={characterForm.combatPower} onChange={(event) => setCharacterForm((current) => ({ ...current, combatPower: sanitizeNumericInput(event.target.value) }))} placeholder="예: 85000000" /></label><div className="form-actions"><button type="submit">{editingCharacterId ? "저장" : "추가"}</button>{editingCharacterId ? <button className="secondary-button" type="button" onClick={resetCharacterForm}>취소</button> : null}</div></form></div></div> : null}</div>;
}
